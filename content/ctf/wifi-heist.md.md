...
Kigges der på `it_guy_scramble()` funktionen
```python
ef it_guy_scramble(password):
    """This is how the IT guy hides the WiFi password."""
    # STEP 1: Shift every character forward by 3
    # (each letter becomes the letter 3 places ahead)
    shifted = []
...
Ses det at der itereres over hvert bogstav, og for hvert bogstav sker følgende:
- Først findes UTF-8 tallet for hvert bogstav og adderes med 3. 
- Dernæst findes bogstavet af det nye tal, og tilføjes til listen `shifted`
- `shifted` bliver vendt om
- Hvert bogstav XOR'es med 42. 

Det er blot derfor at gøre alt dette men i omvendt rækkefølge: 
- Hvert bogstav XOR'es med 42. 
- `shifted` bliver vendt om
- UTF-8 tallet for hvert bogstav og subtraheres med 3.
- bogstavet af det nye tal, og tilføjes til listen `shifted`

Dette blev implementeret i en funktion kaldt `it_guy_descramble()`
```python
def it_guy_descramble(scrambled_password):
   #  det præcis modsatte skal gøres, og dermed starte med STEP 3 i scrambling og unscramble
    # Så først XOR vi hvert tal i listen i scrambled_passwords.
    backwards = scrambled_password
    magic_number = 42
    unmixed = []
    for char in backwards:
        unmixed.append(char ^ magic_number)
    # Dernæst skal vi vende om på listen
    backwards = unmixed[::-1]
    # Til sidst skal vi nu "deshift" altså rykke 3 karakterer tilbage
    deshifted = []
    for char in backwards:
        deshifted.append(chr(char - 3))
    return "".join(deshifted)
```
Hvis denne bliver brugt til at descramble passwordet i listen fås flaget: 
